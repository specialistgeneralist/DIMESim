function [T, grouping_vars] = make_exps_table2(RUNF)
...
% Modified: TM, 17 May 2024 (Curtin)
...
if exist('variable', 'var')
    ...
    for i = 1:lvars        
        ...
        if strcmpi(tname, 'pFnuR')
            t2_varnames = {'p' 'F' 'nu' 'R'};
        else
            t2_varnames = {tname};
        end
        T = [T array2table(t2, 'VariableNames', t2_varnames)];                
    end
    T = unique(T, 'rows');      % .. hierarchy by var 1 .. k
    n = height(T);
    
    grouping_vars = string({variable(~ismember({variable(:).name}, 'rng_seed')).name});
    gv_i = find(ismember(grouping_vars, 'pFnuR'));
    if ~isempty(gv_i) % split pFnuR into individual params
        grouping_vars = [grouping_vars(1:gv_i - 1) "p" "F" "nu" "R" grouping_vars(gv_i + 1:end)];
    end
end
